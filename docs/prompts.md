...
关键命令：
  ...
  ./go-quant forward validate   验证前向测试毛收益、成本与净收益
...
5. 实时行情是否提示高开、涨幅偏高、涨停风险、盘中走弱或卖压确认
...
（如果不需要实时行情，加 --realtime=false 跳过；
默认先用东方财富，主源失败或覆盖不足时自动切换到新浪；两个源都不可用时退回本地数据。）

请用"昨天收盘后的量化信号 + 实时行情"做盘中修正，并报告实际使用的来源。
重点判断：
...
```
请只做盘中可交易性校验，不要重写历史数据，也不要把实时价当成正式日线回测数据。
如果实时行情请求失败，请明确说明“无法做盘中修正”，然后退回使用本地最新 Tushare 日线和新闻/资金数据分析。
```
...
```
验证前向测试记录的收益表现（短线回填 1/3/5 日，中线回填 10/20/40 日，长线回填 60/120/250 日；每期同时记录毛收益、成本拖累和净收益）：
...
./go-quant forward validate
优先对比 1 日/3 日/5 日净收益，检验短线信号扣除双边手续费和滑点后的准确性与稳定性。
```
...
```
一键日终流程（拉数据 → 板块快照 → 信号 → 自动重建验证证据 → 精简推荐）：
1. go build -o go-quant ./cmd/go-quant/
2. ./go-quant daily --ai
   （--ai 可选：追加 AI 点评，需 config.yaml 启用 ai 并设置 QUANT_AI_API_KEY）
   默认输出 Top 3 买入推荐 + 3 条观察机会；可用 -n / --watch 调整
3. 分析输出：市场概况 + 仓位策略 + 买入推荐/观察机会 + 卖出回避 + 持仓盈亏 + 新闻热点
4. 重点校验：是否应该空仓 + 多策略共振 + 流动性 + 资金确认/背离 + 新闻/政策催化 + 涨跌停不可成交风险
5. 如果仓位策略是空仓或观望，正式推荐买入写"无"，观察机会只能写"可跟踪/等待确认"，不要为了凑满 Top N 强行推荐
6. 整理成一段简洁的日报：今日市场判断、是否空仓、推荐买入的股票、观察机会、我的持仓操作建议（日报行数最好不超过60行）

注意：PE、PE_TTM、PB、ROE、分红与行业估值不属于日终交易条件；不要执行 `fetch --financials` 或 `value` 命令，也不要把价值候选混入正式交易推荐。

历史验证说明：正式买入资格现在以"跑赢等权市场代理基准"为准（期望收益 - 同窗口代理收益 > min_alpha_pct，默认 0.5%，且跑赢折数达标，且日期簇等权回撤不超过 max_drawdown_pct 默认 -45%），不再要求绝对正收益；弱市中跑赢市场的策略仍可正式推荐。证据覆盖全部市场市况和短/中/长三个周期。若市况非空头但没有任何合格候选，系统会放行"轻仓试错"（单票仓位≤3%），这些试错会进入前向测试验证，用真实收益决定去留——不要把它们当成正式推荐。`--validate=false` 可跳过证据重建加速调试。
```

```
两周复盘（验证周期：2026-08-06 起每天 16:00 后跑 ./go-quant daily，至 2026-08-24 验证）：
1. 先执行：./go-quant forward validate（回填所有已过观察期的毛/净收益）
2. 读取 data/forward_test/picks.csv 分析（只看信号日期在 2026-08-06 ~ 2026-08-24 的记录）：
   - 共推荐多少只票、多少笔已回填（status 为 validated_*）、多少笔还在观察期
   - 推荐命中率、平均毛收益/净收益、盈亏比、最大单笔亏损、成本占收益比例
   - CASH（观望/空仓）天数占比；对比 MARKET_PROXY_EQUAL_WEIGHT 基准：空仓是躲过了亏损还是踏空了行情
   - 收益按市场状态（market_status 列）分层，哪些状态下推荐有效、哪些失效
   - 观察机会里有没有随后兑现的股票（用于判断 watchlist 是否值得跟踪）
3. 输出 15 行内复盘：结论 + 是否继续每天跑 daily + 是否调整（策略组合/推荐数量/买入门槛）+ 是否启用 AI 点评
```

```
盘中获取当前价格并修正建议：
./go-quant signal -n 5 --watch 15
请重点看输出里的“盘中”列，基于实时当前价判断（记录实际数据来源）：
1. 候选股当前价、盘中涨跌幅、更新时间
...
4. 如果实时行情已经有开盘数据，哪些票高开过多、涨停买不进或盘中走弱
5. 我的持仓按实时价该不该动
6. 新闻/政策有没有明显风险或催化
7. 一句话市场判断
```
