...
历史验证说明：正式买入资格现在以"跑赢等权市场代理基准"为准（期望收益 - 同窗口代理收益 > min_alpha_pct，默认 0.5%，且跑赢折数达标），不再要求绝对正收益；弱市中跑赢市场的策略仍可正式推荐。证据覆盖全部市场市况和短/中/长三个周期。若市况非空头但没有任何合格候选，系统会放行"轻仓试错"（单票仓位≤3%），这些试错会进入前向测试验证，用真实收益决定去留——不要把它们当成正式推荐。`--validate=false` 可跳过证据重建加速调试。
```
