...
5. ./go-quant fetch --daily-basic --today（用于当天换手率；如果接口数据还没出可跳过，报告会标记缺失）
6. ./go-quant fetch --index --today
7. ./go-quant signal -n 10 --watch 5 （仅在09:30-11:30、13:00-15:00自动拉取约1分钟的全市场行情：东方财富为主、覆盖不足或异常时新浪降级；同一快照复用到候选、观察机会和持仓当前价；其他时段使用Tushare日线）
8. 分析输出：市场概况 + A股赚钱效应/涨跌停扩散 + 新闻热度 + 仓位策略 + 持仓实时盈亏 + 短线/中线/长线交易信号 + 观察机会榜
9. 重点校验：是否应该空仓 + 多策略共振 + 流动性/预计成交占比 + 观察机会是否值得跟踪 + 资金确认/背离 + 实时当前价/盘中涨跌幅 + 新闻/政策催化 + 涨跌停不可成交风险 + A股风险标签
10. 如果仓位策略是空仓或观望，正式推荐买入写"无"，观察机会只能写"可跟踪/等待确认"，不要为了凑满 Top N 强行推荐
11. 整理成一段简洁的日报：今日市场判断、是否空仓、推荐买入的股票、观察机会、我的持仓操作建议（日报行数最好不好超过60行，字数太多影响观感）

注意：PE、PE_TTM、PB、ROE、分红与行业估值不属于日终交易条件；`fetch --daily-basic --today` 只为读取换手率，不要执行 `fetch --financials` 或 `value` 命令，也不要把价值候选混入正式交易推荐。
```
