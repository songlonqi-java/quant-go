...
3. 资金流向是否出现资金确认、资金背离、资金分歧
4. 新浪实时行情是否提示高开、涨幅偏高、涨停风险、盘中走弱或卖压确认
5. 新闻热度里是否有政策、监管、行业、公司或市场级别的大事件
6. 强势板块和弱势板块是否与新闻/政策互相印证
7. 如果信号中包含我已持有的股票，提醒是加仓、减仓、止盈、止损还是继续持有
8. 明日开盘或盘中是否存在涨停买不进、跌停卖不出的风险
输出时请明确区分：量化信号、资金流向、盘中实时价格、新闻/政策催化、风险提示、最终建议。
```
...
1. 多策略共振
2. 资金确认/背离/分歧
3. 新浪实时行情显示的盘中价格、涨跌幅和交易可行性标签
4. 新闻热度和政策/行业/公司重大事件
5. 涨跌停不可成交风险
6. 我的持仓操作建议
7. 短线/中线/长线候选是否已写入 forward_test，方便后续按周期验证
```
...
6. 最终只给“可低吸观察、等待回落、放弃追高、风险确认”这类盘中动作建议
```
...
```
请基于 ./go-quant signal -n 5 的输出做新闻和政策校验。
要求：
...
7. 分析输出：市场概况 + A股赚钱效应/涨跌停扩散 + 新闻热度 + 仓位策略 + 持仓实时盈亏 + 短线/中线/长线交易信号
8. 重点校验：是否应该空仓 + 多策略共振 + 资金确认/背离 + 新浪当前价/盘中涨跌幅 + 新闻/政策催化 + 涨跌停不可成交风险 + A股风险标签
...
4. 哪些昨天的短线候选仍可低吸，哪些应该放弃追高
5. 如果仓位策略为“空仓”或“观望”，直接给出无买入建议
```
...
只告诉我：
1. 今天是否应该空仓
2. 如果不是空仓，今天该买什么（Top 5）
3. 如果新浪实时行情已经有开盘数据，哪些票高开过多、涨停买不进或盘中走弱
4. 我的持仓按实时价该不该动
5. 新闻/政策有没有明显风险或催化
6. 一句话市场判断
```
