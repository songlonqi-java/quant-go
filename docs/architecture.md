...
`internal/backtest` 负责单票回测、共享资金组合回放和绩效统计。`PortfolioEnvironment` 拥有排序后的行情副本、日期索引、确定性代码顺序和历史市场状态；这些不可变输入可以在消融两组间复用，但策略实例、缓存和账户状态不得共用。策略消融在该模块内计算基线/实验组总指标、半边换手、逐年稳定性和严格准入门禁；CLI 只选参和打印，不另写绩效口径。
