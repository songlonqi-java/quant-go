...
`--ablation` 会从基线策略列表中移除目标策略，再用一组全新策略实例把它加回实验组，避免两次回测共享状态。报告对比净收益、最大回撤、夏普、半边换手率和逐年收益改善次数。它是研究工具，不读取当前 `evidence.json`，也不会自动修改默认策略。

全量消融会连续回放两次共享资金组合，命令会分别提示“运行基线”和“运行实验组”。开发时可先用 `-s relative_strength,atr_breakout,trend_pullback` 做端到端冒烟，但策略准入必须回到完整默认组合和多年区间。
