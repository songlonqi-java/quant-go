...
默认模式逐只股票、逐策略独立满仓，只用于诊断指标本身。`--ensemble` 才会使用共享现金账户，逐日复用多策略聚合、历史市场状态、买入资格、Top-N、已有持仓暴露以及组合/单票/行业预算。组合模式示例：
...
回测成交口径：T 日收盘产生信号，T+1 开盘成交，并考虑滑点、手续费以及开盘涨跌停不可成交约束。有精确 `stk_limit` 时只使用精确价格，缺失时按主板/创业板与科创板/北交所的 10%/20%/30% 规则保守兜底。组合回测中的卖出订单遇到停牌或跌停会延迟到首次可成交开盘；买入订单只尝试下一市场交易日。
...
该命令回放“策略信号 → 市场仓位策略 → 次日可成交入场”完整链路，在历史后半段按时间顺序切分样本外折，并按短、中、长周期记录策略组合与市场状态的胜率、期望收益、波动和最大回撤。入场会拒绝一字涨停和高开超过 3% 等开盘时已知的不可执行场景；开盘后跌破前低只记录为风险，不会反向抹除已经成交的样本。退出也不会越过指定回测截止日。

证据默认写入 `data/raw/validation/evidence.json`，并被 `signal` 读取。文件包含策略与费用、完整行情数据和构建过程指纹；数据补齐、策略、参数、费用或决策实现变更后必须重新构建。验证启用时，证据缺失、过期或不兼容会让正式买入失效关闭。历史抓取现在会覆盖退市和暂停上市证券，但旧归档必须重新执行 `fetch` 才能补齐，且仍不能替代持续的 `forward validate` 前向测试。
