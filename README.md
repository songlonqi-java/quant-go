...
详见 [docs/strategies.md](docs/strategies.md)。`relative_strength` 因持续拖累 alpha 已移出默认组合，实现保留为研究项。原 24 个策略的重叠关系、统一诊断结果和 11 个退役项见 [docs/todo/strategy-autopsy.md](docs/todo/strategy-autopsy.md)。保留不代表策略已经被证明有效；新增策略仍需先用组合消融比较净收益、回撤、换手和跨年稳定性。
